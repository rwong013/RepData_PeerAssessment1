...
qplot(data=activitybyday, steps, geom = "histogram")
```
...
qplot(data=activityfillbyday, steps, geom = "histogram")
```

Using the filled in data, the mean total number of steps taken each day is `r signif(meanstepsfill, digits = 5)` whereas previously it was `r meansteps`.
